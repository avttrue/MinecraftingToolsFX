...
/**
 * Хэндлер выхода игрока с сервера
 */
public class UserQuitHandler implements UnpackedObjectTypeHandler
{
    ...
    {
        logger.log(Level.INFO, String.format("UserQuit.create: %1$d", objectId));
        ...
        mcMessage.message = String.format("ВЫШЕЛ: в игре %1$s, на сервере %2$s", player_tp, player_sp);
            ...
            Map <Integer, DataValue> packedProperties)
    {
        ...
        logger.log(Level.INFO, String.format("UserQuit.update: %1$d", objectId));
    ...
    {
        logger.log(Level.INFO, String.format("UserQuit.destroy: %1$d", objectId));
