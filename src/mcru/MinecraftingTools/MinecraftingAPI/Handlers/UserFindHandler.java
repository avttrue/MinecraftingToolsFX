...
/**
 * Хэндлер поиска игрока на сервере: "name" для поиска по имени и "uuid" для поиска по ююиду
 */
public class UserFindHandler implements UnpackedObjectTypeHandler
{
    ...
    {
        logger.log(Level.INFO, String.format("UserFind.create: %1$d", objectId));
...
//            System.out.println("Property.Key: " + entry.getKey() + ", Value: " + entry.getValue());
            ...
            Map <Integer, DataValue> packedProperties)
    {
        ...
        logger.log(Level.INFO, String.format("UserFind.update: %1$d", objectId));
    ...
    {
        logger.log(Level.INFO, String.format("UserFind.destroy: %1$d", objectId));
